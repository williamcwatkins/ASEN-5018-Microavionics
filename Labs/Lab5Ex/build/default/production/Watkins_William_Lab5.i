# 1 "Watkins_William_Lab5.asm"
# 1 "<built-in>" 1
# 1 "Watkins_William_Lab5.asm" 2
;;;;;; ASEN 5067 LAB 5 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
# 171 "Watkins_William_Lab5.asm" 2
...
Initial:
    ...
    BSF RCON, 7, a ; Set ((RCON) and 0FFh), 7, a bit <7>
...
;;;;;; Alive LED/PWM Output Setup
    ...
    BSF INTCON, 6, a ; ((INTCON) and 0FFh), 6, a bit<6> enables low-priority interrupts
    BSF INTCON, 7, a ; ((INTCON) and 0FFh), 7, a bit<7> enables high-priority interrupts
...
;;;;;; Begin LED sequence
    BSF LATD,5,A ; Turn on ((PORTE) and 0FFh), 5, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait05s ; 1s delay subroutine
    BCF LATD,5,A ; Turn off ((PORTE) and 0FFh), 5, a

    BSF LATD,6,A ; Turn on ((PORTE) and 0FFh), 6, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait05s ; 1s delay subroutine
    BCF LATD,6,A ; Turn off ((PORTE) and 0FFh), 6, a

    BSF LATD,7,A ; Turn on ((PORTE) and 0FFh), 7, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait05s ; 1s delay subroutine
    BCF LATD,7,A ; Turn off ((PORTE) and 0FFh), 7, a
...
Wait10ms:
        BTFSS PIR2,1,A ; Read Timer3 ((PIR2) and 0FFh), 1, a rollover flag and ...
        ...
        BCF PIR2,1,A ; Clear Timer3 ((PIR2) and 0FFh), 1, a rollover flag
        RETURN
...
HiPriISR: ; High-priority interrupt service routine
        ...
        BRA HP1
        RCALL CCP1handler ; Call CCP1handler for blinking ((PORTD) and 0FFh), 4, a LED
        BRA HiPriISR
HP1:
        BTFSS PIR3, 2, a ; Test ((PIR3) and 0FFh), 1, a bit <1> for this interrupt
        BRA HP2
        RCALL CCP3handler ; Call CCP1handler for blinking ((PORTD) and 0FFh), 4, a LED
        BRA HP1
HP2:
...
CCP1Test:
 ...
 BRA LoPriCCP1
HiPriCCP1: ; LED just turned ON - set ((PORTC) and 0FFh), 2, a to trigger when it should turn off
 BSF LATD, 4, a ; Manually toggle ((PORTD) and 0FFh), 4, a
 MOVF low AliveON, w, a ; and add half period to CCPR1 to add more pulse time
        ...
        ADDWFC CCPR1X, f, a
 BRA CCP1Exit
LoPriCCP1: ; LED just turned OFF - set ((PORTC) and 0FFh), 2, a to trigger when it should turn on
 BCF LATD, 4, a ; Manually toggle ((PORTD) and 0FFh), 4, a
...
CCP3handler: ; First must test of ((PIR1) and 0FFh), 0, a occurred at the same time
        BTFSS PIR1, 0, a ; If TMR1's overflow flag is set? skip to test CCP bit7
...
CCP3Test:
 ...
 BRA LoPriCCP3
HiPriCCP3: ; PWM just turned ON - set ((PORTC) and 0FFh), 1, a to trigger when it should turn off
 BSF LATC, 2, a
 MOVF low PWMON, w, a ; and add half period to CCPR1 to add more pulse time
        ...
        ADDWFC CCPR3X, f, a
 BRA CCP3Exit
LoPriCCP3: ; PWM just turned OFF - set ((PORTC) and 0FFh), 1, a to trigger when it should turn on
 BCF LATC, 2, a
 MOVF low PWMOFF, w, a ; and add half period to CCPR1 to add more pulse time
        ...
        ADDWFC CCPR3X, f, a
CCP3Exit:
        BCF PIR4, 0, a ; Clear the ((PIR3) and 0FFh), 1, a bit <1> interrupt flag
