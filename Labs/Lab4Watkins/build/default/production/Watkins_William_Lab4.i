# 1 "Watkins_William_Lab4.asm"
# 1 "<built-in>" 1
# 1 "Watkins_William_Lab4.asm" 2
;;;;;;; ASEN 5067 LAB 4 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
# 169 "Watkins_William_Lab4.asm" 2
...
loop:
    ...
    RCALL Check_SW1 ; Call Check_SW1 to check if ((PORTE) and 0FFh), 3, a has been
...
;;;;;; Begin LED sequence
    BSF LATE,5,A ; Turn on ((PORTE) and 0FFh), 5, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait1s ; 1s delay subroutine
    BCF LATE,5,A ; Turn off ((PORTE) and 0FFh), 5, a

    BSF LATE,6,A ; Turn on ((PORTE) and 0FFh), 6, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait1s ; 1s delay subroutine
    BCF LATE,6,A ; Turn off ((PORTE) and 0FFh), 6, a

    BSF LATE,7,A ; Turn on ((PORTE) and 0FFh), 7, a
    BSF T0CON,7,A ; Start 1s timer
    RCALL Wait1s ; 1s delay subroutine
    BCF LATE,7,A ; Turn off ((PORTE) and 0FFh), 7, a
...
;;;;;; 250ms Timer 0 Setup
    ...
    BSF LATC,2,A ; Turn on ((PORTC) and 0FFh), 2, a pin for PWM signal
...
;
; Subroutine to check the status of ((PORTE) and 0FFh), 3, a button and change PW
...
debouncelp:
    ...
    BTFSC PORTE, 0x3, A ; Check if ((PORTE) and 0FFh), 3, a has been released and...
...
LCD1:
    ...
    BCF PIR5,1,A ; Clear Timer5 ((PIR5) and 0FFh), 1, a rollover flag
...
fine:
    ...
    BSF LATC,2,A ; Set ((PORTC) and 0FFh), 2, a to high
    CLRF PW+10,A
    RETURN
...
;;;;;;; PWMOff subroutine ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; This subroutine turns off the output to ((PORTC) and 0FFh), 2, a, resets the Timer 5 overflow flag,
...
; if a bit is set in PWM, and turns on the output to ((PORTC) and 0FFh), 2, a.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

PWMOn:
    MOVLF high Bignumdos,TMR1H,A ; Write the timer values into
    MOVLF low Bignumdos,TMR1L,A ; the timer high and low registers
    BCF PIR1,0,A ; Clear Timer1 ((PIR1) and 0FFh), 0, a rollover flag
